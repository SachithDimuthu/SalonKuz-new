 ...
 * Edit Deal
 * 
 * This page allows administrators to edit an existing deal or discount.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if service already has an active deal (only if changing service)
    if ($serviceId > 0 && $serviceId !== $deal['service_id']) {
        $existingDeal = $dealModel->getActiveDealForService($serviceId);
        
        if ($existingDeal) {
            $errors[] = 'This service already has an active deal. Please select a different service.';
        }
    }
    
    // If no errors, update the deal
    if (empty($errors)) {
        $updated = $dealModel->updateDeal(
            $dealId,
            $name,
            $description,
            $serviceId,
            ...
            <div class="md:w-3/4 md:pl-6">
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        ?>
                        <div class="mb-6">
                            <span class="<?php echo $statusClass; ?> text-sm px-3 py-1 rounded-full capitalize">
                                <?php echo $status; ?>
                            </span>
                        </div>
                        
                        <!-- Edit Deal Form -->
                        <form action="edit-deal.php?id=<?php echo $dealId; ?>" method="POST">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <!-- Deal Name -->
                                <div class="md:col-span-2">
                                    <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Deal Name *</label>
                                    <input type="text" name="name" id="name" value="<?php echo isset($name) ? $name : $deal['name']; ?>" class="w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring focus:ring-pink-500 focus:ring-opacity-50" required>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="description" id="description" rows="4" class="w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring focus:ring-pink-500 focus:ring-opacity-50" required><?php echo isset($description) ? $description : $deal['description']; ?></textarea>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <select name="service_id" id="service_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring focus:ring-pink-500 focus:ring-opacity-50" required>
                                        ...
                                        <?php foreach ($services as $svc): ?>
                                            <?php 
                                            // Check if service already has an active deal (except this one)
                                            $existingDeal = ($svc['id'] !== $deal['service_id']) ? $dealModel->getActiveDealForService($svc['id']) : null;
                                            $disabled = $existingDeal ? 'disabled' : '';
                                            $selected = (isset($serviceId) ? $serviceId === $svc['id'] : $deal['service_id'] === $svc['id']) ? 'selected' : '';
                                            ?>
                                            <option value="<?php echo $svc['id']; ?>" <?php echo $selected; ?> <?php echo $disabled; ?>>
                                                <?php echo $svc['name']; ?> (<?php echo formatPrice($svc['price']); ?>)
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <input type="number" name="discount_percentage" id="discount_percentage" min="1" max="100" value="<?php echo isset($discountPercentage) ? $discountPercentage : $deal['discount_percentage']; ?>" class="w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring focus:ring-pink-500 focus:ring-opacity-50 pr-12" required>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="start_date" id="start_date" value="<?php echo isset($startDate) ? $startDate : $deal['start_date']; ?>" class="w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring focus:ring-pink-500 focus:ring-opacity-50" required>
                                </div>
                                
                                <!-- End Date -->
                                <div>
                                    <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">End Date *</label>
                                    <input type="date" name="end_date" id="end_date" value="<?php echo isset($endDate) ? $endDate : $deal['end_date']; ?>" class="w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring focus:ring-pink-500 focus:ring-opacity-50" required>
                                </div>
                            </div>
